...
function computePriceMatch(price: number, budget?: number, purpose?: TravelPurpose): number {
  if (!budget || budget <= 0) return 0.5;
  const diff = Math.abs(price - budget);
  let match = 1 - Math.min(diff / budget, 1);
  if (purpose === "budget" && price <= budget) match = Math.min(1, match + 0.15);
...
function computeStarMatch(stars: number, preferred?: number, purpose?: TravelPurpose): number {
  const target = preferred ?? (purpose === "luxury" ? 4 : purpose === "budget" ? 3 : undefined);
  if (!target) return 0.5;
  if (stars >= target) return 1;
  return Math.max(0, 1 - (target - stars) / target);
}

function computeAmenitiesMatch(
  hotelSlugs: string[],
  preferred?: string[],
  purpose?: TravelPurpose
): number {
...
function buildMatchReasons(
  ...
  breakdown: { priceMatch: number; amenitiesMatch: number }
): string[] {
  const reasons: string[] = [];
  if (prefs.budget && breakdown.priceMatch >= 0.7) reasons.push("Matches your budget");
  ...
  if (breakdown.amenitiesMatch >= 0.6 && prefs.amenities?.length) {
    ...
    if (matched.length > 0) reasons.push(`Has ${matched.slice(0, 3).join(", ")}`);
    ...
    if (matched.length > 0) reasons.push(`Ideal for ${prefs.purpose} travel`);
  }
  if (hotel.rating >= 8) reasons.push("Highly rated by guests");
  if (reasons.length === 0) reasons.push("Popular choice in this destination");
...
export function scoreHotel(
  ...
  const userRatingNorm = Math.max(0, Math.min(1, hotel.rating / 10));
  const popularityNorm = Math.max(0, Math.min(1, hotel.popularityScore / 100));
  ...
  const maxWeight =
    weights.price + weights.star + weights.amenities + weights.userRating + weights.popularity;
  const score = Math.round((weighted / maxWeight) * 100);
  const matchReasons = buildMatchReasons(hotel, prefs, { priceMatch, amenitiesMatch });
